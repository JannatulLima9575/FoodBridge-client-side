...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (!/[A-Z]/.test(password)) return toast.error("Must include an uppercase letter");
    if (!/[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]/.test(password)) return toast.error("Must include a special character");
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-center gap-8 min-h-screen">
      ...
      <div className="w-full md:w-1/2 p-6 md:p-10 max-w-md bg-white dark:bg-[#2c2c2c] rounded-xl shadow-md">
        <h2 className="text-2xl font-bold mb-6 text-[#1e1e1e] dark:text-white">Create your FoodBridge account</h2>
        <form onSubmit={handleRegister} className="space-y-4">
          <input name="name" type="text" placeholder="Full Name" className="input input-bordered w-full dark:bg-[#333] dark:text-white" required />
          ...
          <input name="confirmPassword" type="password" placeholder="Confirm Password" className="input input-bordered w-full dark:bg-[#333] dark:text-white" required />
        ...
        <p className="mt-4 text-sm text-gray-500">
          Already have an account? <Link to="/login" className="text-[#F9A825] font-medium">Login</Link>
        </p>
      </div>
    </div>
  );
